...
public class ListingsTableModel extends AbstractTableModel {
    ...
    public String getColumnName(int column) {
        try {
            String colName = rs.getMetaData().getColumnName(column + 2);
    ...
    public Object getValueAt(int row, int column) {
        try {
            ...
            if(column == 2)
                return "(" + rs.getObject(column + 2) + ") " + rs.getObject(column + 3) + "-" + rs.getObject(column + 4);
            else
                return rs.getObject(column + 2);
        } catch (SQLException e) {
            e.printStackTrace();
            return null;
        }
